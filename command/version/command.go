...
func (cmd *Version) Execute(args []string) error {
	...
	if cmd.Automation {
		...
		if !cmd.Verbose {
			// short
			info = &ShortInfo{
				Name:        strings.TrimPrefix(build.Name, "cloudctl-"),
		...
		if err != nil {
			zap.S().With(zap.Error(err)).Error("error marshalling plugin info to JSON")
			return err
		}
		zap.S().Debug("marshalling data to JSON", string(data))
